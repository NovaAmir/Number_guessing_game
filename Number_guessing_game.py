...
async def start_game(update : Update , context : CallbackContext) -> int:
    try:
        ...
        if min_num >= max_num:
            await update.message.reply_text("Minimum must be smaller than maximum. Try again!")
            return GET_MIN
...
async def finish(update:Update , context:CallbackContext) -> int:
    ...
    await update.message.reply_text(emoji.emojize(f"Excellent! The number was {true_num} . You guessed it in {count} tries."))
